...
public class AddNewHybridCar extends Command {
    @Override
    public void action() {
        StaticDataDisplayer.hybridPoweredCarHelper();
        boolean isFullData = false;
        while (!isFullData) {
            try {
                VehicleType vehicleType = VehicleType.HYBRID_CAR;
                ...
                String[] vehicleData = new String[] {vehicleType.toString(), carMark, modelName, String.valueOf(engineDisplacement),
                        String.valueOf(powerOfTheEngine), String.valueOf(capacityOfTheBattery), String.valueOf(price)};
